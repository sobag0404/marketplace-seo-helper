...
import { generateHashtags } from '@/lib/marketplace/hashtagGenerator';

interface QuickSuggestionsProps {
  /** Current hashtags string (space-separated) for the row */
  currentHashtags: string;
  /** The selected preset rules */
  preset: Preset;
...
export function QuickSuggestions({
  ...
  const suggestions = useMemo(() => {
    ...
    // Generate all potential hashtags for this product
    const generated = generateHashtags(productName, preset);
    if (!generated) return [];

    const allGenerated = generated.split(' ').filter(Boolean);
    ...
    const filtered = allGenerated.filter((tag) => !currentSet.has(tag));

    // Take up to 10 suggestions
    return filtered.slice(0, 10);
  }, [productName, preset, currentHashtags]);
